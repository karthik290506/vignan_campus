...
def get_weather(latitude=17.3331139, longitude=78.718202):
    ...
    #codes to determine weather conditions
...
def show_header():
    logo_path = os.path.join("C:/", "Users", "K.Naga Harshitha", "Downloads", "vignan_logo.png")
    try:
...
def show_academics(data):
    """Academics page with PDF upload feature for admins"""
                ...
                if st.form_submit_button("Upload PDF"):
                    if pdf_file is not None:
                        ...
                        if existing_pdf:
                            ...
                            st.rerun() # Rerun to display the new PDF
                    else:
            ...
            for pdf in data["academic_pdfs"]:
                if os.path.exists(pdf["file_path"]):
                    ...
                    st.download_button(
                        ...
                        file_name=os.path.basename(pdf["file_path"]),
                        mime="application/pdf",
                        key=f"download_pdf_public_{pdf['file_path']}" # Unique key for public view
                    )
                else:
                    st.warning(f"File not found: {pdf['subject']}")
        else:
